...
- [x] 3.5 Delete the test tag/release created in 3.4 once verified, so it doesn't linger as a real release — repo owner deleted both manually from GitHub (this session's git push access is scoped to the feature branch and can't delete remote tags, and no GitHub MCP tool deletes a tag or release); confirmed gone via the API (both `v0.0.1-test.1`'s tag ref and its Release now 404)
